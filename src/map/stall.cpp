...
*/
int8 stall_ui_open(struct map_session_data* sd, uint16 skill_lv, short type){
	nullpo_retr(1, sd);
 ...
 */
int8 stall_vending_setup(struct map_session_data* sd, const char* message, const int16 xPos, const int16 yPos, uint8 *data, int count)
{
	int i, j, k, l;
 ...
 * @return 0 If success, 1 - Cannot open (die, not state.prevend, trading), 2 - No cart, 3 - Count issue, 4 - Cart data isn't saved yet, 5 - No valid item found
 */
int8 stall_buying_setup(struct map_session_data* sd, const char* message, const int16 xPos, const int16 yPos, const struct STALL_BUYING_SET_sub* itemlist, int count, uint64 total_price)
{
 ...
 */
void stall_vending_listreq(struct map_session_data* sd, int id)
{
	nullpo_retv(sd);
 ...
 * @param id : vender account id (gid)
 */
void stall_buying_listreq(struct map_session_data* sd, int id)
{
	nullpo_retv(sd);
	struct s_stall_data* st;
 ...
 */
void stall_vending_purchasereq(struct map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
	int i, w;
	double z;
 ...
 * @param count : number of different items he's trying to buy
 */
void stall_buying_purchasereq(struct map_session_data* sd, int aid, int uid, const struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub* itemlist, unsigned int count )
{
	int zeny = 0;
	struct s_stall_data* st = map_id2st(uid);
...
void stall_close(struct map_session_data* sd){
	auto itStalls = std::find_if(stall_db.begin(), stall_db.end(), [&](s_stall_data *const & itst) {
 ...
 */
bool stall_searchall(struct map_session_data* sd, const struct s_search_store_search* s, const struct s_stall_data* st, short type)
{
	int c, slot;
	unsigned int cidx;
