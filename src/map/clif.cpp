...
void clif_parse_PurchaseReq2(int32 fd, map_session_data* sd){
	...
	if (p->UniqueID >= START_STALL_NUM) {
		...
		clif_msg_color(*sd, MSI_CANNOT_ACCESS_BY_WEIGHTOVER_80, color_table[COLOR_CYAN]);
	}
	else
...
static void clif_parse_ReqTradeBuyingStore( int32 fd, map_session_data* sd ){
	...
	if (p->storeId >= START_STALL_NUM) {
		...
		clif_msg(*sd, MSI_STORE_ASSISTANT_REUTRN_DATE);
	}
	else
...
#else
	p.bodypalette = st->vd.look[LOOK_CLOTHES_COLOR];
	p.BackAccessory = st->vd.look[LOOK_ROBE];
#endif
