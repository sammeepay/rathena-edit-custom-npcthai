	...
	case SP_GOLDPC_POINTS:
		...
		if( !sd->state.connect_new ){
			...
			if( battle_config.feature_goldpc_active && val < battle_config.feature_goldpc_max_points && !sd->state.autotrade ){
				...
				sd->goldpc_tid = add_timer(gettick() + ((battle_config.feature_goldpc_time - remaining_seconds) * 1000 > 0 ? (battle_config.feature_goldpc_time - remaining_seconds) * 1000 : battle_config.feature_goldpc_time * 1000), pc_goldpc_update, sd->bl.id, (intptr_t)nullptr);
				}
